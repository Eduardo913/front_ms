...
Future<String> buscar_pokemon(String id) async {
  final response =
      await http.get(Uri.parse("http://localhost:5000/api/pokemon/$id"));
...
Future<String> suma_http(String number1, String number2) async {
  final response = await http.get(Uri.parse(
      "http://localhost:5000/api/calculadora/suma/$number1/$number2/"));
  return response.body;
}

Future<String> resta_http(String number1, String number2) async {
  final response = await http.get(Uri.parse(
      "http://localhost:5000/api/calculadora/resta/$number1/$number2/"));
...
Future<String> multiplicacion_http(String number1, String number2) async {
  final response = await http.get(Uri.parse(
      "http://localhost:5000/api/calculadora/multiplicacion/$number1/$number2/"));
